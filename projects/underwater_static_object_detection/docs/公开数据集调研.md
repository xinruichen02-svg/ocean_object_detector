...
DUO 贴近水下机器人采摘、底栖生物识别和目标检测任务，包含：
...
| --- | --- | --- | --- | --- | --- |
| DUO | 已选定 | holothurian, echinus, scallop, starfish | 检测 | COCO；以本地 JSON 复核 | 第一版正式数据集 |
...
- 地址：https://github.com/chongweiliu/DUO
- 任务：水下目标检测。
- 相关类别：holothurian、echinus、scallop、starfish。
- 适合程度：高。
- 确定用途：第一版水下静态物体检测 baseline。
- 需要确认：
  - 本地下载包的真实目录结构。
  - COCO JSON 文件名、类别 ID 和图片路径。
  - 训练集和测试集划分方式。
  - 是否已有独立验证集。
